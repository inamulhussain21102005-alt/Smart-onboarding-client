...
function TasksPage({ user, tasks, setTasks, employees, addToast, addActivity, G, settings, sound }) {
  const [tab, setTab] = useState("tasks"); // "tasks" | "workflows"
  ...
  const [assigningWf, setAssigningWf] = useState(null); // taskId being assigned workflow
  ...
  // Load workflows when workflows tab is opened or when assigning
  useEffect(() => {
    if (tab === "workflows" || assigningWf) {
      setWfLoading(true);
      api.getWorkflows()
  ...
  const assignWorkflowToEmployee = async (wf, empId) => {
    try {
      await api.assignWorkflow(wf._id, empId);
      const emp = employees.find(e => e._id === empId);
      addToast(`Workflow "${wf.name}" assigned to ${emp?.name}!`, "success");
      addActivity(`Workflow "${wf.name}" → ${emp?.name}`, "workflow_assign");
      playSound('deploy', sound);
      setAssigningWf(null);
    } catch { addToast("Failed to assign workflow", "error"); }
  };
    ...
    <div style={{ animation: "fadeUp 0.3s ease" }}>
      {wfLoading ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 11 }}>
          {[1, 2, 3].map(i => (
            <div key={i} className="glass-card" style={{ padding: 18, height: 160 }}>
              <div style={{ background: G.border, borderRadius: 6, height: 12, width: "60%", marginBottom: 10 }} />
              <div style={{ background: G.border, borderRadius: 6, height: 8, width: "80%", marginBottom: 7 }} />
              <div style={{ background: G.border, borderRadius: 6, height: 8, width: "50%" }} />
            </div>
          ))}
        </div>
      ) : workflows.length === 0 ? (
        <EmptyState icon="🔄" title="No workflows yet" subtitle="Add workflows via API to assign them here" G={G} />
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 12 }}>
          {workflows.map((wf, i) => {
            const [selEmp, setSelEmp] = useState("");
            return (
              <div key={wf._id} className="glass-card" style={{ padding: 18, animation: `fadeUp 0.4s ease ${i * 0.07}s both` }}>
                {/* Header */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <div style={{ fontFamily: G.font, fontSize: 11, fontWeight: 700, marginBottom: 5 }}>{wf.name}</div>
                    <Bdg label={wf.department || "All Depts"} color={G.secondary} />
                  </div>
                  <div style={{ background: `${G.primary}15`, color: G.primary, border: `1px solid ${G.primary}33`, borderRadius: 6, padding: "3px 7px", fontSize: 9, fontFamily: G.font, fontWeight: 700 }}>
                ...
                {/* Steps list */}
                <div style={{ marginBottom: 12 }}>
                  {wf.steps?.slice(0, 4).map((s, j) => (
                    <div key={j} style={{ display: "flex", alignItems: "center", gap: 7, padding: "4px 0", borderBottom: `1px solid ${G.border}` }}>
                      <div style={{ width: 16, height: 16, borderRadius: 4, background: `${G.primary}15`, color: G.primary, fontSize: 8, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, flexShrink: 0 }}>{s.order}</div>
                  ...
                  {wf.steps?.length > 4 && (
                    <div style={{ color: G.muted, fontSize: 10, paddingTop: 4, textAlign: "center" }}>+{wf.steps.length - 4} more steps</div>
                  )}
                </div>
                ...
                {isAdmin && (
                  <div>
                    <div style={{ marginBottom: 6 }}>
                      <label style={{ color: G.muted, fontSize: 9, display: "block", marginBottom: 4, fontFamily: G.font, letterSpacing: 1 }}>ASSIGN TO EMPLOYEE</label>
                      <select
                        className="p-input"
                        value={selEmp}
                    ...
                    <SBtn G={G} onClick={() => { if (selEmp) assignWorkflowToEmployee(wf, selEmp); else addToast("Select an employee first", "error"); }} style={{ width: "100%", borderRadius: 8 }}>
                      ⚡ Assign Workflow
                    </SBtn>
                  </div>
                )}
  ...
  return (
    <div style={{ animation: "slideLeft 0.4s ease" }}>
      ...
      {/* Page Header */}
      <PageHdr title="Tasks & Workflows" subtitle={`${visible.length} tasks · ${workflows.length} workflows`} G={G}
        action={
          <div style={{ display: "flex", gap: 8 }}>
            {isAdmin && tab === "tasks" && <SBtn G={G} onClick={() => setShowForm(!showForm)}>+ New Task</SBtn>}
          </div>
        }
      />

      {/* Tab switcher */}
      <div style={{ display: "flex", gap: 4, marginBottom: 16, background: G.surface, border: `1px solid ${G.border}`, borderRadius: 12, padding: 4, width: "fit-content" }}>
        {[
          ...
          <button
            key={t.key}
            onClick={() => { setTab(t.key); playSound('click', sound); }}
            style={{
              display: "flex", alignItems: "center", gap: 6,
              padding: "7px 16px",
              background: tab === t.key ? G.primary : "transparent",
              color: tab === t.key ? "#fff" : G.muted,
              border: "none", borderRadius: 9,
              fontSize: 12, fontWeight: 700,
              cursor: "pointer", fontFamily: G.font,
              transition: "all 0.2s",
            }}
          >
            <span>{t.icon}</span>
            <span>{t.label}</span>
            {t.key === "tasks" && (
              <span style={{ background: tab === t.key ? "rgba(255,255,255,0.25)" : `${G.primary}22`, color: tab === t.key ? "#fff" : G.primary, borderRadius: 20, padding: "0px 6px", fontSize: 9, fontFamily: G.font }}>
                {visible.length}
              </span>
            )}
          </button>
        ...
        <>
          {showForm && (
            <div className="glass-card" style={{ padding: 20, marginBottom: 16, border: `1px solid ${G.orange}22`, animation: "fadeUp 0.3s ease" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 14 }}>
                <div style={{ width: 3, height: 13, background: G.orange, borderRadius: 2 }} />
                <h3 style={{ fontFamily: G.font, color: G.orange, fontSize: 10, letterSpacing: 2 }}>NEW TASK</h3>
              </div>
              <div className="form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <label style={{ color: G.muted, fontSize: 9, display: "block", marginBottom: 4, fontFamily: G.font }}>Title *</label>
                  <input className="p-input" placeholder="Task title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
                </div>
                <div>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                <GBtn G={G} color={G.orange} onClick={addTask}>Create Task</GBtn>
                <GBtn G={G} color={G.muted} onClick={() => setShowForm(false)}>Cancel</GBtn>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {filtered.length === 0 ? (
              <EmptyState icon="📋" title="No tasks found" subtitle={filter === "all" ? "Create your first task" : `No ${filter.replace("_", " ")} tasks`} G={G} />
            ) : filtered.map((t, i) => {
              const dl = getDL(t.dueDate);
              const isExpanded = expandedTask === t._id;
              return (
                <div key={t._id} className="glass-card" style={{ animation: `fadeUp 0.4s ease ${i * 0.05}s both`, border: `1px solid ${tC[t.status]}18`, transition: "all 0.3s" }}>
                  {/* Task row */}
                  <div style={{ padding: 15, display: "flex", gap: 11, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <div style={{ marginLeft: "auto", display: "flex", gap: 6, alignItems: "center" }}>
                          {/* Workflow assign button */}
                          {isAdmin && (
                            <GBtn size="sm" G={G} color={G.secondary} onClick={() => setExpandedTask(isExpanded ? null : t._id)}>
                  ...
                  {isExpanded && isAdmin && (
                    <div style={{ borderTop: `1px solid ${G.border}`, padding: "14px 15px", background: `${G.secondary}05`, animation: "fadeUp 0.25s ease" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 12 }}>
                        <div style={{ width: 3, height: 12, background: G.secondary, borderRadius: 2 }} />
                        <span style={{ fontFamily: G.font, fontSize: 9, fontWeight: 700, color: G.secondary, letterSpacing: 2 }}>ASSIGN WORKFLOW TO {t.assignedTo?.name?.toUpperCase() || "EMPLOYEE"}</span>
                      </div>
                      {wfLoading ? (
                        <div style={{ color: G.muted, fontSize: 12 }}>Loading workflows...</div>
                      ) : workflows.length === 0 ? (
                        <div style={{ color: G.muted, fontSize: 12 }}>No workflows available. Add via API.</div>
                      ) : (
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          {workflows.map(wf => (
                            <div key={wf._id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: G.card, border: `1px solid ${G.border}`, borderRadius: 9 }}>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 12, fontWeight: 600 }}>{wf.name}</div>
                                <div style={{ color: G.muted, fontSize: 10, marginTop: 2 }}>{wf.steps?.length || 0} steps · {wf.department || "All depts"}</div>
                              </div>
                              <GBtn size="sm" G={G} color={G.secondary}
                                onClick={() => t.assignedTo?._id
                                  ? assignWorkflowToEmployee(wf, t.assignedTo._id)
                                  : addToast("Task has no assigned employee", "error")
                                }>
                                Assign
                              </GBtn>
                            </div>
